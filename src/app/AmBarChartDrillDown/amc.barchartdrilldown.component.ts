...
@Component({
  selector: 'app-amc-bar-chart-drill-down',
  template: `<div id="chartdiv5" [style.width.%]="100" [style.height.px]="420"></div>`
})
export class AmcBarChartDrillDownComponent implements AfterViewInit, OnDestroy {
  chartData: any;
  ...
  // this.AmCharts.addListener(this.chart, 'clickGraphItem', (event) => {
  //   if ('object' === typeof event.item.dataContext.months) {
  //     // set the monthly data for the clicked month
  //     event.chart.dataProvider = event.item.dataContext.months;
  //     // update the chart title
